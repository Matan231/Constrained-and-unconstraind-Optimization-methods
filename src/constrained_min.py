...
def interior_pt(func, x0, ineq_constraints, eq_constraints_mat= np.array([None]), eq_constraints_rhs=np.array([None])):
    ...
    while True:
        if len(ineq_constraints) > 0:
            print(t)
            func = partial(aug_func, func, t, ineq_constraints)

        if eq_constraints_mat.all() == None:
            max_iterations = 100
            step_tol = 10 ** (-12)
            obj_tol = 10 ** (-12)
            x_t, suc, path = line_search(func, x_t, obj_tol, step_tol, max_iterations, dir_selection_method='nt', back_track_factor=0.5)
        else:
            print("sgd")
            x_t, suc, path = const_nt_decent(func, x_t, eq_constraints_mat, eq_constraints_rhs, e)
    ...
    return x_t, total_path


def aug_func(func, t, ineq_constraints, x, hessian=True):

    f_x, df_x, hessian_f_x = func(x)
    ineq_g = []
    ineq_dg = []
    ineq_hes_g = []
...
def const_nt_decent(func, x0, eq_constraints_mat, eq_constraints_rhs, e=10 ** (-7), init_step_len=1, max_iter=100, slope_ratio=np.e**(-4), back_track_factor=0.5):
    path = []
    step_size = init_step_len
    x_prev = x0

    f_prev, df_prev, hessian_prev = func(x_prev)
    i = 0
    success = False
    ...
    while not success and i < max_iter:
        ...
        f_next, df_next, hessian_next = func(x_next)
